...
const updateScreen = async () => {
  ...
  titleInfo.similar_titles.slice(0, 5).forEach(async (title, i) => {
    console.log(title)
    // let first = await fetch(`https://api.watchmode.com/v1/title/${title}/details/?apiKey=${watchModeKey}&append_to_response=sources`)
    // let firstData = await first.json()
    let firstData = testData
    let response = await fetch(`https://imdb-api.com/en/API/SearchTitle/k_vd8o64bn/${firstData.imdb_id}`)

    let data = await response.json()
    const image = document.createElement('img')
    image.setAttribute('src', data.results[0].image)
    image.classList = 'cursor-pointer'
    document.querySelector('.pic-ctn').append(image)
    image.addEventListener('click', () => {
      document.querySelector('.pic-ctn').innerHTML = ''
      titleInfo = firstData
      updateScreen()
    })
  })
